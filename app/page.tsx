...
export default function Page() {
  ...
  return <main>
    ...
    <section id="birthday" className="birthday-section" onMouseEnter={() => setConfetti(true)}><div className="birthday-inner"><Reveal><span className="eyebrow">The celebration</span><h2>Chapter One: <em>Your First Birthday</em></h2><p className="birthday-note">A little cake, a lot of love, and a room full of people who came just to celebrate you.</p></Reveal><div className="birthday-grid"><PhotoPlaceholder label="BIRTHDAY / CAKE" tone="peach"/><div className="birthday-details"><div><Cake size={19}/><span><b>{baby.birthday}</b>Birthday date</span></div><div><Clock3 size={19}/><span><b>{baby.location}</b>Birthday location</span></div><div><Heart size={19}/><span><b>[Birthday theme]</b>Birthday theme</span></div><p>Happy birthday, little one. May your life always be filled with gentle mornings, loud laughter, and people who show up for every chapter.</p></div></div>{confetti && <div className="confetti" aria-hidden="true">{Array.from({length: 16}).map((_,i)=><i key={i} style={{'--i': i} as React.CSSProperties}/>)}</div>}</div></section>
    ...
    <footer><span>Made with love for {baby.name}.</span><span>Chapter 01 • Age One • {baby.year}</span><span>More memories coming...</span><a href="/admin">Private editor</a></footer>
