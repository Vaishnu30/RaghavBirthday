...
export default function AdminPage() {
  ...
  async function saveRow(row: any) { setSaving(true); const { error } = await supabase.from(active.table).update({ title: row.title, name: row.name, caption: row.caption, description: row.description, message: row.message, relationship: row.relationship, published: row.published, updated_at: new Date().toISOString() }).eq('id', row.id); if (error) setError('Unable to save this memory.'); else setEditingId(null); setSaving(false) }
  ...
  if (user && !authorized) return <main className="admin-gate"><div className="admin-login"><ShieldCheck size={32} /><span className="eyebrow">Private family archive</span><h1>Access is restricted</h1><p>This account is signed in, but it has not been added to the family archive.</p><button type="button" onClick={() => supabase.auth.signOut()}>Sign out</button></div></main>
  return <main className="admin-shell"><header className="admin-header"><div><span className="eyebrow">Raghav&apos;s time capsule</span><h1>Memory book editor</h1></div><button className="admin-logout" onClick={() => supabase.auth.signOut()}><LogOut size={16} /> Sign out</button></header><div className="admin-layout"><aside className="admin-sidebar"><p>Manage</p>{sections.map(item => { const Icon = item.icon; return <button className={active.key === item.key ? 'selected' : ''} key={item.key} onClick={() => setActive(item)}><Icon size={16} />{item.label}</button> })}</aside><section className="admin-content"><div className="admin-toolbar"><div><span className="eyebrow">{active.label}</span><h2>Keep the story growing</h2></div><button onClick={addPlaceholder} disabled={saving}><Plus size={16} /> Add memory</button></div><div className="admin-list">{rows.length ? rows.map(row => <article key={row.id}>{editingId === row.id ? <div className="admin-edit-form"><input aria-label="Title" value={row.title || row.name || row.caption || ''} onChange={e => setRows(rows.map(item => item.id === row.id ? { ...item, [row.title !== undefined ? 'title' : row.name !== undefined ? 'name' : 'caption']: e.target.value } : item))} /><textarea aria-label="Description" value={row.description || row.message || row.relationship || ''} onChange={e => setRows(rows.map(item => item.id === row.id ? { ...item, [row.message !== undefined ? 'message' : row.description !== undefined ? 'description' : 'relationship']: e.target.value } : item))} /><div><button onClick={() => saveRow(row)} disabled={saving}><Save size={15} /> Save</button><button onClick={() => setEditingId(null)}>Cancel</button></div></div> : <><div><b>{row.title || row.name || row.caption || `Age ${row.age}` || 'Memory'}</b><small>{row.description || row.relationship || row.message || row.status || 'Ready to edit'}</small></div><button aria-label="Edit memory" onClick={() => setEditingId(row.id)}><Save size={15} /></button></>}</article>) : <div className="empty-admin"><Sparkles size={25} /><h3>This chapter is waiting to be written.</h3><p>Add the first {active.label.toLowerCase()} memory to begin.</p><button onClick={addPlaceholder}><Plus size={15} /> Add memory</button></div>}</div></section></div></main>
}
